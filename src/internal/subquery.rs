    ...
    VAL: 'a,
{
    ...
    /// 添加自定义列
    pub fn columns(mut self, column_build_fn: impl FnOnce(&mut SubqueryBuilder<'_, VAL>)) -> Self {
        if self.has_from {
            return self;
        }
    ...
    /// Add JOIN clause
    pub fn join(mut self, join_type: JoinType, table: impl Into<String>, on_condition: impl FnOnce(&mut SubqueryBuilder<'_, VAL>)) -> Self {
        if !self.has_from {
            self.add_from_clause();
        }
    ...
    pub fn having(mut self, condition: impl FnOnce(&mut SubqueryBuilder<'_, VAL>)) -> Self {
        if !self.has_group_by {
            return self;
        }
        if !self.has_having {
