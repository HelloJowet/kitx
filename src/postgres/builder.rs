//! PostgreSQL query builder module
//!
//! This module provides type aliases and utilities for building PostgreSQL database queries.
...
//! 使得以类型安全的方式处理 PostgreSQL 特定的数据库操作更加容易。

use sqlx::{Postgres, QueryBuilder};

use crate::{
    internal::{delete_builder, insert_builder, select_builder, subquery, update_builder, upsert_postgres},
    postgres::kind::DataKind,
};

/// QueryBuilder type alias for PostgreSQL  
/// PostgreSQL 的 QueryBuilder 类型别名
///
/// This is a type alias for sqlx::QueryBuilder<'a, Postgres>, which is used to build dynamic SQL queries
/// in a type-safe manner for PostgreSQL databases. It provides methods to push SQL fragments, bind parameters,
/// and construct complex queries programmatically.
///
/// 这是 sqlx::QueryBuilder<'a, Postgres> 的类型别名，用于以类型安全的方式为 PostgreSQL 数据库构建动态 SQL 查询。
/// 它提供了推送 SQL 片段、绑定参数和以编程方式构造复杂查询的方法。
///
/// # Examples
///
/// ```
/// use kitx::postgres::builder::QB;
///
...
///
/// ```
/// use kitx::postgres::builder::{SQB, Subquery};
///
...
///
/// ```
/// use kitx::postgres::builder::{Subquery, Select};
///
...
///
/// Used to build INSERT statements for PostgreSQL databases. Provides methods to insert single or multiple
/// records in a type-safe manner.
///
/// 用于为 PostgreSQL 数据库构建 INSERT 语句。提供以类型安全的方式插入单个或多个记录的方法。
///
/// # Type Parameters
///
...
/// * `returning` - Add RETURNING clause to the insert statement
...
/// * `returning` - 添加 RETURNING 子句到插入语句
/// * `finish` - 结束构建，获取内部的 QueryBuilder
///  
/// # Examples
///
/// ```
/// use kitx::postgres::builder::Insert;
///
...
///
/// Used to build UPDATE statements for PostgreSQL databases. Provides methods to update records with
...
///
/// Used to build DELETE statements for PostgreSQL databases. Provides methods to delete records by
/// primary key or with custom WHERE clauses.
///
/// 用于为 PostgreSQL 数据库构建 DELETE 语句。提供按主键或使用自定义 WHERE 子句删除记录的方法。
///
/// # Type Parameters
///
...
/// * `returning` - Add RETURNING clause to the DELETE statement
/// * `finish` - Finish building, get the internal QueryBuilder
///
/// # 公共方法
///
...
/// * `returning` - 添加 RETURNING 子句到删除语句
/// * `finish` - 结束构建，获取内部的 QueryBuilder
///
/// # Examples
///
/// ```
/// use kitx::postgres::builder::Delete;
///
...
///
/// Used to build SELECT statements for PostgreSQL databases. Provides methods to construct queries
/// with flexible WHERE clauses, JOINs, ORDER BY, LIMIT and other SQL features.
///
/// 用于为 PostgreSQL 数据库构建 SELECT 语句。提供构造具有灵活 WHERE 子句、JOIN、ORDER BY、LIMIT 和其他 SQL 特性的查询的方法。
///
/// # Type Parameters
///
...
///
/// ```
/// use kitx::postgres::builder::Select;
///
...
///
/// Used to build UPSERT statements for PostgreSQL databases (using INSERT ... ON CONFLICT syntax).
/// Provides methods to insert records or replace them if they already exist based on primary key.
///
/// 用于为 PostgreSQL 数据库构建 UPSERT 语句（使用 INSERT ... ON CONFLICT 语法）。
...
///
/// ```
/// use kitx::postgres::builder::Upsert;
///
/// let entity = User { id: 1, name: "Charlie".to_string() };
/// let upsert_query = Upsert::one(&entity, &PRIMARY_KEY).unwrap();
/// ```
pub type Upsert<'a, ET> = upsert_postgres::Upsert<'a, ET, Postgres, DataKind>;
...
mod tests {
    ...
    use crate::{
        ...
        postgres::{
            builder::{Delete, Insert, QB, Select, Subquery, Update, Upsert},
            connection,
            kind::DataKind,
    ...
    async fn init_pool() {
        init_logger();
        let database_url = dotenv::var("POSTGRES_DATABASE_URL").expect("POSTGRES_DATABASE_URL must be set");
    ...
    async fn test_lifecycle() {
        ...
        // Reset sequence to ensure IDs start at 1
        let qb_seq = QB::new("ALTER SEQUENCE article_id_seq RESTART WITH 1");
    ...
    async fn step_find_list_paginated() {
        ...
        let qb2 = Select::<Article>::table().columns(column_build_fn).filter(filter_build_fn).finish();
    ...
    async fn step_find_list_by_cursor() {
        ...
        // 初始请求（无游标）
        let cursor_qb = Select::<Article>::table().cursor(column_key, Order::Asc, None, limit);
        ...
        let cursor_qb2 = Select::<Article>::table().cursor(column_key, Order::Asc, next_cursor, limit);
        ...
        // 验证排序逻辑（降序测试）
        let cursor_qb_desc = Select::<Article>::table().cursor(column_key, Order::Desc, None, limit);
    ...
    async fn step_with_cte() {
        ...
        let qb = Select::<Article>::from_query_with_table(cte_builder, "article_cte").finish();
