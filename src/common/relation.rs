        ...
        match self.rel_type {
            RelationType::OneToOne => {
                ...
                if values[0] != *&self.primary_key {
                    return Err(RelationError::ValueMismatch(0, format!("{:?}", &self.primary_key), format!("{:?}", &values[0])).into());
                }
            }
            ...
            RelationType::OneToMany => {
                ...
                for (i, value) in values.iter().enumerate() {
                    if value != &self.primary_key {
                        return Err(RelationError::ValueMismatch(i, format!("{:?}", &self.primary_key), format!("{:?}", value)).into());
                    }
                }
            }
