...
/// // values: ["John", "john@example.com", ""]
/// ```
pub fn get_values<ET, VAL>(entity: &ET, columns: &[&str]) -> Vec<VAL>
where
    ET: FieldAccess,
...
/// ```
pub fn get_value<ET, VAL>(entity: &ET, column: &str) -> VAL
where
    ET: FieldAccess,
    VAL: ValueConvert + Default,
{
    entity.field_as_any(column).map(|value| VAL::convert(value)).unwrap_or_default()
}
