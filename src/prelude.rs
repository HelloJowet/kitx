pub use crate::common::error::{QueryError, RelationError};
pub use crate::common::fields::{batch_extract, extract_all, extract_with_bind, extract_with_filter, get_value, get_values};
pub use crate::common::filter::{push_primary_key_bind, push_primary_key_conditions};
pub use crate::common::helper::{QueryCondition, get_table_name};
pub use crate::common::relation::EntitiesRelation;
pub use crate::common::types::{CursorPaginatedResult, Order, PaginatedResult, PrimaryKey};

#[cfg(feature = "sqlite")]
pub mod sqlite {
    pub use crate::sqlite::{
        builder::{Delete, Insert, QB, SQB, Select, Subquery, Update, Upsert},
        connection::{create_db_pool, setup_db_pool},
        kind::DataKind,
        query::{execute, execute_with_trans, fetch_all, fetch_one, fetch_optional, fetch_scalar, fetch_scalar_optional},
    };
}

#[cfg(feature = "mysql")]
pub mod mysql {
    pub use crate::mysql::{
        builder::{Delete, Insert, QB, SQB, Select, Subquery, Update, Upsert},
        connection::{create_db_pool, setup_db_pool},
        kind::DataKind,
        query::{execute, execute_with_trans, fetch_all, fetch_one, fetch_optional, fetch_scalar, fetch_scalar_optional},
    };
}

#[cfg(feature = "postgres")]
pub mod postgres {
    pub use crate::postgres::{
        builder::{Delete, Insert, QB, SQB, Select, Subquery, Update, Upsert},
        connection::{create_db_pool, setup_db_pool},
        kind::DataKind,
        query::{execute, execute_with_trans, fetch_all, fetch_one, fetch_optional, fetch_scalar, fetch_scalar_optional},
    };
}
